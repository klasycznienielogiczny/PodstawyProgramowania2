...
/** Tresc zadania:
Ćw3 [17pkt] Klasy z dynamiczną zawartością, wyjątki (UWAGA: To bardzo wazne zadanie, jeśli ktoś chce być programista C++
...
Nasza implementacja w razie automatycznego zwiekszania rozmiaru ma allokowac pamiec tylko o 1 wieksza!
    ...
    Wszystkie atrybuty powinny być prywatne, konstruktory i metody - publiczne, metody/konstruktory/destruktory większe
    niż 1-linijkowe powinny być zadeklarowane w klasie, zdefiniowane poza klasą,
...
private:
    Fraction *data;
    size_t _capacity; //rozmiar zaalokowanej
    size_t _size; //ilosc elementow
public:
    size_t size() const;
    size_t capacity() const;

    Vector(size_t rozmiar=0);
